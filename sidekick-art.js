/* Science Sidekick drawings — the Portal's own sheet (science-coach-art.js),
   loaded beside index.html so a Scan card and a Portal card wear the same figure.
   An unknown id falls back to Comparison Casey in the Portal; Scan never draws
   that fallback: sidekickAvatarHtml refuses an id the taxonomy does not know. */
...
var SCIENCE_COACH_IDS = Object.freeze([
...
function renderScienceCoachAvatar(id,{animated=true}={}) {
  const key = typeof id === 'string' && Object.prototype.hasOwnProperty.call(CAST,id) ? id : 'comparison';
  const c = CAST[key];
